...
{
    public class Fragment_fav_ads : Fragment
    {
        ...
        public Fragment_fav_ads(string nm)
        {
            this.nm = nm;
        }
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            View myView = inflater.Inflate(Resource.Layout.fav_ads_fragment, container, false);
            help = new DBHelper(Activity);
            myListView = myView.FindViewById<ListView>(Resource.Id.list_favads);
            mySearchView = myView.FindViewById<SearchView>(Resource.Id.search_favads);

            ICursor result = help.showAllfavAds(nm);
        ...
        private const string Column_ad_email = "ad_email"; */
                ...
                var idFromDb = result.GetString(result.GetColumnIndexOrThrow("f_id"));
                var ad_titleFRomDb = result.GetString(result.GetColumnIndexOrThrow("f_ad_title"));
                var ad_DescFRomDb = result.GetString(result.GetColumnIndexOrThrow("f_ad_description"));
                var priceFromDb = result.GetString(result.GetColumnIndexOrThrow("f_price"));
                var regionFromDb = result.GetString(result.GetColumnIndexOrThrow("f_region"));
